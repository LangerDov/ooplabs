...
namespace ooplab_4
{
    public partial class Form1 : Form
    {
        ...
        class CGroup : gOBJ
        {
            ...
            public void LoadChilds(TreeNode node)
            {
                for (int i = 0; i < childs.Count; i++)
                {
                    ...
                    {
                        case CCircle:
                            new_node = node.Nodes.Add("CCircle");
                            new_node.Tag = childs[i];
                            break;
                        case CGroup:
                            new_node = node.Nodes.Add("CGroup");
                            ((CGroup)childs[i]).LoadChilds(new_node);
                            new_node.Tag = childs[i];
                            break;
                        case CLine:
                            new_node = node.Nodes.Add("CLine");
                            new_node.Tag = childs[i];
                            break;
                        case CRect:
                            new_node = node.Nodes.Add("CRect");
                            new_node.Tag = childs[i];
                            break;
                    }
        ...
        private void UpdateTV()
        {
            ...
            for (int i = 0; i < circles.Count; i++)
            {
                ...
                {
                    case CCircle:
                        node = treeView1.Nodes.Add("CCircle");
                        node.Tag = circles[i];
                        break;
                    case CGroup:
                        node = treeView1.Nodes.Add("CGroup");
                        ((CGroup)circles[i]).LoadChilds(node);
                        node.Tag = circles[i];
                        break;
                    case CLine:
                        node = treeView1.Nodes.Add("CLine");
                        node.Tag = circles[i];
                        break;
                    case CRect:
                        node = treeView1.Nodes.Add("CRect");
                        node.Tag = circles[i];
                        break;
                }
            }
        ...
        private void treeView1_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            selected_circles.Clear();
            selected_circles.Add((gOBJ)e.Node.Tag);

            ((gOBJ)e.Node.Tag).SetSelected();
            this.Invalidate(true);
        }
    }
}